...
dv_init_dirs
dv_log "=== STEP 6: Publication-style TRA plots ==="

for f in \
  "${DIR_SUMMARY}/per_sample_TRA_counts.tsv" \
  "${DIR_SUMMARY}/per_chromosome_TRA_counts.tsv" \
  "${DIR_SUMMARY}/pairwise_shared_TRA.tsv" \
  "${DIR_SUMMARY}/TRA_binary_genotype_matrix.tsv" \
  "${DIR_FINAL}/catalog_226.TRA.GT_matrix.tsv"; do
  dv_check_file "$f" "$(basename "$f")"
done

"${RSCRIPT_BIN}" "${SCRIPT_DIR}/../utils/plot_TRA_results.R" \
