...
dv_init_dirs
dv_log "=== STEP 2: Per-sample DELLY TRA discovery ==="
...
run_discovery() {
  ...
  local out_bcf="${DIR_DISC}/${sid}.disc.TRA.bcf"
  ...
  echo "[$(date '+%T')] ${sid}: starting TRA discovery..."
...
while IFS= read -r sid; do
  [[ -f "${DIR_DISC}/${sid}.disc.TRA.bcf" ]] || { dv_err "Missing: ${sid}"; ((FAIL++)) || true; }
done < "$SAMPLES_ALL"
...
{
  echo -e "sample\tn_TRA"
  for bcf in "${DIR_DISC}"/*.disc.TRA.bcf; do
    sid=$(basename "$bcf" .disc.TRA.bcf)
    n=$(bcftools view -H "$bcf" 2>/dev/null | wc -l)
    echo -e "${sid}\t${n}"
  done
} | tee "${DIR_LOGS}/discovery_TRA_counts.tsv"
