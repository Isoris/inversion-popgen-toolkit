#!/usr/bin/env Rscript
# =============================================================================
# 06_plot_tra_results.R
# Publication-style TRA catalog summary plots
#
# Panels:
#   A — Genome-wide TRA density heatmap (1-Mb windows, per sample; breakpoint 1)
#   B — Per-sample TRA count barplot
#   C — PCA on binary TRA genotype matrix
#   D — Pairwise shared TRA heatmap
#   F — Per-chromosome TRA count barplot
#
# Notes:
#   - No Panel E for TRA/BND because there is no simple single SVLEN analogue
...
per_sample <- fread(file.path(opt$summary_dir, "per_sample_TRA_counts.tsv"))
per_chr    <- fread(file.path(opt$summary_dir, "per_chromosome_TRA_counts.tsv"))
pairwise   <- fread(file.path(opt$summary_dir, "pairwise_shared_TRA.tsv"))
binary_gt  <- fread(file.path(opt$summary_dir, "TRA_binary_genotype_matrix.tsv"))
...
cat("  TRA sites:", nrow(binary_gt), "\n")

sample_order <- per_sample[order(-n_SVs), sample]

# =============================================================================
# PANEL A: Genome-wide TRA density heatmap per sample (breakpoint 1 only)
# =============================================================================
cat("=== Panel A: Genome-wide heatmap ===\n")

gt_matrix <- fread(file.path(opt$final_dir, "catalog_226.TRA.GT_matrix.tsv"))
samples <- colnames(gt_matrix)[6:ncol(gt_matrix)]

# Use first breakpoint only: CHROM + POS
...
if (nrow(window_sample_dt) == 0) {
  stop("No sample-window counts available for Panel A")
}
...
col_fun_a <- colorRamp2(c(0, 1, 2, 4), c("#440154", "#31688e", "#35b779", "#fde725"))
...
make_ht_a <- function() {
  Heatmap(
    ...
    column_title = "A  Genome-wide TRA density (1-Mb windows; breakpoint 1)",
    column_title_gp = gpar(fontsize = 12, fontface = "bold"),
    row_title = "Samples",
    use_raster = TRUE,
    raster_quality = 3,
    heatmap_legend_param = list(title = "# TRA", title_gp = gpar(fontsize = 8))
  )
}
...
# =============================================================================
# PANEL B: Per-sample TRA count barplot
# =============================================================================
...
p_b <- ggplot(per_sample, aes(x = sample, y = n_SVs, fill = source)) +
  ...
  labs(x = "Sample", y = "TRA count", title = "B") +
  theme_pub +
  theme(
...
ggsave(file.path(opt$plot_dir, "panel_B_per_sample_TRA.pdf"), p_b, width = 16, height = 5, dpi = 400)
ggsave(file.path(opt$plot_dir, "panel_B_per_sample_TRA.png"), p_b, width = 16, height = 5, dpi = 400)
cat("  Panel B saved.\n")

# =============================================================================
# PANEL C: PCA on binary TRA genotype matrix
# =============================================================================
cat("=== Panel C: PCA ===\n")
...
cat("  PCA input:", nrow(pca_input), "samples x", ncol(pca_input), "TRAs\n")
...
if (ncol(pca_input) > 1) {
  ...
  ggsave(file.path(opt$plot_dir, "panel_C_PCA_TRA.pdf"), p_c, width = 7, height = 6, dpi = 400)
  ggsave(file.path(opt$plot_dir, "panel_C_PCA_TRA.png"), p_c, width = 7, height = 6, dpi = 400)
  cat("  Panel C saved.\n")
} else {
...
# =============================================================================
# PANEL D: Pairwise shared TRA heatmap
# =============================================================================
...
make_ht_d <- function() {
  Heatmap(
    ...
    column_title = "D  Pairwise shared TRAs",
    column_title_gp = gpar(fontsize = 12, fontface = "bold"),
    use_raster = TRUE,
    raster_quality = 3
  )
...
# =============================================================================
# PANEL F: Per-chromosome TRA counts
# =============================================================================
...
p_f <- ggplot(per_chr, aes(x = chrom_label, y = n_SVs)) +
  ...
  labs(x = "Chromosome", y = "TRA count", title = "F") +
  theme_pub +
  theme(
...
if (exists("p_c")) {
  plots_list <- c(plots_list, list(p_c + theme(legend.position = "none")))
}
plots_list <- c(plots_list, list(p_f))

composite <- plot_grid(plotlist = plots_list, ncol = 2)
ggsave(file.path(opt$plot_dir, "composite_TRA_BCF.pdf"), composite, width = 16, height = 8, dpi = 400)
ggsave(file.path(opt$plot_dir, "composite_TRA_BCF.png"), composite, width = 16, height = 8, dpi = 400)
cat("  Composite BCF saved.\n")

legend_file <- file.path(opt$plot_dir, "figure_legend.txt")
legend_lines <- c(
  "Figure X. Structural translocation candidates in catfish hatchery cohort.",
  "",
  paste0("(A) Genome-wide distribution of TRA breakpoints across ", length(samples),
         " samples. Heatmap shows TRA count per non-overlapping 1-Mb window using breakpoint 1. ",
         "Top barplot: cumulative TRA count. Right barplot: per-sample total."),
  paste0("(B) Per-sample TRA counts, sorted by total. ",
         "Teal = unrelated subset (n=", length(unrelated_samples), ")."),
  if (exists("p_c")) "(C) PCA based on binary TRA genotype matrix." else "(C) PCA omitted due to insufficient variable sites.",
  "(D) Pairwise shared TRA heatmap with hierarchical clustering (Ward's method).",
  "(F) TRA counts per chromosome using breakpoint 1.",
  "",
  "Note: No Panel E is included for TRA because BND-style translocations do not have a simple single-length analogue like DEL/INV."
)
