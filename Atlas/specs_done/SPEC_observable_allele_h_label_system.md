...
**Status**: SHIPPED in turns 139 (classifier slice 1) + 140 (chip).
See `tests/test_turn139_h_label_classifier_slice1.js` and
`tests/test_turn140_h_label_chip.js`. Drafted turn 137, rewritten turn 138 to
flip the ordering (observe first → derive system size → label, instead of
assume K → label → check). Replaces the implicit H-label semantics in
`_KARYO_DETAILED_LABELS` (`Inversion_atlas.html` line ~32940).
