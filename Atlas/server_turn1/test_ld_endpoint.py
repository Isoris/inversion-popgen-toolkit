"""Tests for ld_endpoint.py.

Relocated from server_turn11c_ld_fast/ during turn 145 server-unify.
Requires the fast_ld native binary at engine_fast_ld/fast_ld; tests bail
...
ROOT = Path(__file__).resolve().parent.parent   # bundle root (parent of server_turn1/)
...
def test_request_validation():
    # Valid
    LDSplitReq(chrom="C_gar_LG28", window_range=[0, 10],
              groups={"a": ["S1", "S2", "S3", "S4", "S5"]})

    # window_range bad
    for wr in ([], [5], [0, 1, 2], [10, 5], [-1, 5]):
        try:
            LDSplitReq(chrom="x", window_range=wr,
                      groups={"a": ["S1", "S2", "S3", "S4", "S5"]})
    ...
    # group too small
    try:
        LDSplitReq(chrom="x", window_range=[0, 5],
                  groups={"a": ["S1", "S2"]})
...
def test_cache_key_is_order_independent():
    a = LDSplitReq(chrom="X", window_range=[0, 10],
                  groups={"g": ["S3", "S1", "S2", "S4", "S5"]})
    b = LDSplitReq(chrom="X", window_range=[0, 10],
...
def test_cache_key_changes_with_meaningful_inputs():
    ...
    b = LDSplitReq(chrom="X", window_range=[0, 11],
                  groups={"g": ["S1", "S2", "S3", "S4", "S5"]})
    c = LDSplitReq(chrom="Y", window_range=[0, 10],
                  groups={"g": ["S1", "S2", "S3", "S4", "S5"]})
    d = LDSplitReq(chrom="X", window_range=[0, 10],
                  groups={"g": ["S1", "S2", "S3", "S4", "S5"]},
                  thin_to=200)
...
def test_shape_response_strips_dense_matrices():
    ...
    req = LDSplitReq(chrom="C_gar_LG28", window_range=[0, 10],
