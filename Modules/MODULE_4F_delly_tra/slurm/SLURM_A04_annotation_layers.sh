...
dv_init_dirs
dv_log "=== ANNOTATION LAYERS (TRA) ==="

SV_BED_226="${DIR_FINAL}/catalog_226.TRA.bed"
dv_check_file "${SV_BED_226}" "226 TRA BED"
SV_BED_81=$(ls "${DIR_FINAL}"/catalog_81.TRA.*.PASS.bed 2>/dev/null | head -1)
...
if [[ -f "${ANNOT_BEDS}/REPEATS.sorted.bed" ]]; then
  for label_bed in "catalog_226:${SV_BED_226}" "catalog_81:${SV_BED_81:-}"; do
    ...
    bedtools intersect -a "$bed" -b "${ANNOT_BEDS}/REPEATS.sorted.bed" -wa -f 0.5 -u > "${DIR_ANNOT}/${label}.TRAs_in_repeats.bed" 2>/dev/null || true
    bedtools intersect -a "$bed" -b "${ANNOT_BEDS}/REPEATS.sorted.bed" -wa -f 0.5 -v > "${DIR_ANNOT}/${label}.TRAs_not_in_repeats.bed" 2>/dev/null || true
    dv_log "  ${label}: $(wc -l < "${DIR_ANNOT}/${label}.TRAs_in_repeats.bed") in repeats"
  done
fi

# ── No SVLEN QC for TRA (no meaningful SVLEN) ──────────────────────────
dv_log "--- Skipping SVLEN QC (not applicable for TRA) ---"
