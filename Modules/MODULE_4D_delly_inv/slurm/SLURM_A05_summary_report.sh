...
dv_init_dirs
dv_log "=== FINAL SUMMARY (INV) ==="

REPORT="${DIR_SUMMARY}/delly_INV_summary_report.txt"
COUNTS="${DIR_SUMMARY}/delly_INV_counts.tsv"

# Counts
FINAL_226_VCF="${DIR_FINAL}/catalog_226.INV.vcf.gz"
N_226=0; [[ -f "${FINAL_226_VCF}" ]] && N_226=$(bcftools view -H "${FINAL_226_VCF}" | wc -l)
FINAL_81_PASS=$(ls "${DIR_FINAL}"/catalog_81.INV.*.PASS.vcf.gz 2>/dev/null | head -1)
...
N_REP=0;N_NOREP=0
[[ -f "${DIR_ANNOT}/catalog_226.INVs_in_repeats.bed" ]] && N_REP=$(wc -l < "${DIR_ANNOT}/catalog_226.INVs_in_repeats.bed")
[[ -f "${DIR_ANNOT}/catalog_226.INVs_not_in_repeats.bed" ]] && N_NOREP=$(wc -l < "${DIR_ANNOT}/catalog_226.INVs_not_in_repeats.bed")

# Per-sample + sharing
MATRIX_226="${DIR_FINAL}/catalog_226.INV.GT_matrix.tsv"
PER_SAMPLE="${DIR_SUMMARY}/per_sample_INV_counts.tsv"
PRIV_SHARED="${DIR_SUMMARY}/private_vs_shared_INV.tsv"
PER_CHR="${DIR_SUMMARY}/per_chromosome_INV_counts.tsv"
PAIRWISE_SHARED="${DIR_SUMMARY}/pairwise_shared_INV.tsv"
PCA_INPUT="${DIR_SUMMARY}/INV_binary_genotype_matrix.tsv"
SVLEN_DIST="${DIR_SUMMARY}/INV_svlen_distribution.tsv"
WINDOW_COUNTS="${DIR_SUMMARY}/INV_window_counts_1Mb.tsv"
...
print("Computing pairwise shared INVs...")
...
cat > "${REPORT}" << REOF
================================================================================
DELLY INV CATALOG — SUMMARY REPORT
Generated: $(date '+%F %T')
Reference: fClaHyb_Gar_LG.fa | Cohort: 226 (81 unrelated) | ~9X lcWGS
Type: Inversions
================================================================================
226 catalog:     ${N_226} INVs
81 PASS catalog: ${N_81_PASS} INVs
