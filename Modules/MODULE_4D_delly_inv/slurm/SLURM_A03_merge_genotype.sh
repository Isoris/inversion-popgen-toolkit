...
dv_init_dirs
dv_log "=== STEPS 3–8: Merge → Regenotype → Cohort → Germline filter (INV) ==="
...
run_genotype() {
  ...
  local out="${DIR_GENO}/${sid}.geno.INV.bcf"
...
if "${DELLY_BIN}" filter -f germline -o "${GERMLINE_81}" "${COHORT_81_TRIM}" \
  ...
  dv_log "  Germline PASS: $(bcftools view -H "${GERMLINE_81}" | wc -l) INVs"
else
...
# 226 raw catalog
FINAL_226_RAW_VCF="${DIR_FINAL}/catalog_226.INV.raw.vcf.gz"
bcftools view -O z -o "${FINAL_226_RAW_VCF}" "${COHORT_226}"
tabix -f -p vcf "${FINAL_226_RAW_VCF}"

# 226 strict catalog
FINAL_226_VCF="${DIR_FINAL}/catalog_226.INV.vcf.gz"
...
# 81 raw catalog
FINAL_81_RAW_VCF="${DIR_FINAL}/catalog_81.INV.${LABEL_81}.raw.vcf.gz"
bcftools view -O z -o "${FINAL_81_RAW_VCF}" "${INPUT_81}"
tabix -f -p vcf "${FINAL_81_RAW_VCF}"

# 81 strict catalog
FINAL_81_PASS="${DIR_FINAL}/catalog_81.INV.${LABEL_81}.PASS.vcf.gz"
if [[ "${STRICT_REQUIRE_PASS}" -eq 1 ]]; then
...
MATRIX_81="${DIR_FINAL}/catalog_81.INV.${LABEL_81}.PASS.GT_matrix.tsv"
{
