...
#
# Panels:
#   A — Genome-wide INV density heatmap (1-Mb windows, per sample)
#       with cumulative barplot on top and per-sample total on right
#   B — Per-sample INV count barplot (sorted)
#   C — PCA on binary INV genotype matrix
#   D — Pairwise shared INV heatmap with hierarchical clustering
#   E — INV size distribution (log-scale histogram)
#   F — Per-chromosome INV count barplot
# =============================================================================
...
svlen_dist   <- fread(file.path(opt$summary_dir, "INV_svlen_distribution.tsv"))
window_counts <- fread(file.path(opt$summary_dir, "INV_window_counts_1Mb.tsv"))
...
# =============================================================================
# PANEL A: Genome-wide INV density heatmap per sample
# =============================================================================
cat("=== Panel A: Genome-wide heatmap ===\n")

gt_matrix <- fread(file.path(opt$final_dir, "catalog_226.INV.GT_matrix.tsv"))
...
col_fun_a <- colorRamp2(c(0, 2, 5, 10), c("#440154", "#31688e", "#35b779", "#fde725"))
...
right_anno <- rowAnnotation(
  total = anno_barplot(per_sample[match(sample_order, sample), n_SVs],
                       width=unit(2, "cm"),
...
ht_a <- Heatmap(mat_a,
  ...
  column_title="A  Genome-wide INV density (1-Mb windows)",
  column_title_gp=gpar(fontsize=12, fontface="bold"),
  row_title="Samples",
  use_raster=TRUE,
  raster_quality=3,
  heatmap_legend_param=list(title="# INV", title_gp=gpar(fontsize=8))
)
draw(ht_a)
dev.off()
cat("  Panel A saved.\n")
...
if (exists("p_c")) plots_list <- c(plots_list, list(p_c + theme(legend.position="none")))
plots_list <- c(plots_list, list(p_e, p_f))
...
legend_lines <- c(
  "Figure X. Structural inversion variants in catfish hatchery cohort.",
  "",
  paste0("(A) Genome-wide distribution of inversions across ", length(samples),
         " samples. Heatmap shows INV count per non-overlapping 1-Mb window. ",
         "Top barplot: cumulative INV count. Right barplot: per-sample total."),
  paste0("(B) Per-sample INV counts, sorted by total. ",
         "Teal = unrelated subset (n=", length(unrelated_samples), ")."),
  if (exists("p_c")) paste0("(C) PCA based on binary INV genotype matrix.") else "(C) PCA omitted due to insufficient variable sites.",
  "(D) Pairwise shared INV heatmap with hierarchical clustering (Ward's method).",
  paste0("(E) INV size distribution (log scale). Median size: ",
         round(median(svlen_dist$svlen_bp)), " bp."),
  "(F) INV counts per chromosome/linkage group."
)
