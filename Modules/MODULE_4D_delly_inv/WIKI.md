...
- MODULE_5A discovers inversion candidates from population-level signal (local PCA, GHSL v5 within-sample haplotype divergence, three-snake consensus). These are *statistical* candidates — regions of the genome where the population data behaves as if there is an inversion.
...
MODULE_4D INV catalog
  ...
  │   - 50% reciprocal overlap against three-snake consensus regions
