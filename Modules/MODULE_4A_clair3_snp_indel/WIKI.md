...
6. **The per-sample classification (BC0–BC4) requires SnpEff annotation.** SnpEff databases are reference-specific. MODULE_CON's scoring pipeline depends on a SnpEff database built for the *C. gariepinus* Gar subgenome reference; quality of annotation depends on the gene model quality (`fClaHyb_Gar_LG.from_CGAR.gff3_polished.sorted.gff3`).
