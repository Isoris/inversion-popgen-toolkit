#!/usr/bin/env bash
###############################################################################
# helpers/04_beagles.sh — BEAGLE GL per-RF + whole-genome + merge
...
for W in "${THIN_FINE[@]}"; do
  ...
  echo "[INFO] thin_${W}: sbatch --array=0-$((N-1))%8 helpers/slurm_beagle_rf.sh ${CHUNK_LIST} ${W}"
done
...
for W in "${THIN_BROAD[@]}"; do
  SITES="${THIN_DIR}/03_sites/sites.thin_${W}.majmin.tsv"
  [[ -s "$SITES" ]] || { echo "[WARN] Missing sites for thin_${W}"; continue; }
  echo "[INFO] thin_${W} WG: sbatch helpers/slurm_beagle_wg.sh ${W}"
done

echo ""
echo "[INFO] After all BEAGLE jobs complete, merge per-RF to WG:"
echo "  bash helpers/merge_beagles.sh"
