...
`cheat28_tandem_repeat_context.R` exists in `4f_group_dependent/` and
...
- `4g_final_classification/compute_candidate_status.R::build_key_spec()`
  — the code source of truth.
- `4g_final_classification/characterize_candidate.R` — the per-Q
  convergence functions that consume these keys.
- `4g_final_classification/run_characterize.R` — chat-10 driver.
