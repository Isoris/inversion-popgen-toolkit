...
#
# Also added: cheat27 + cheat28 + cheat29 calls (these 3 exist in 4d but
# the example launcher never called them). They require NONE validation
...
echo "[LAUNCH_group_cheats] PHASE4D_DIR=${PHASE4D_DIR}"
...
for cid in "${CIDS[@]}"; do
  ...
  Rscript "${PHASE4D_DIR}/cheat30_gds_by_genotype.R" \
    --candidate "$cid" \
    --outdir "${RAW_OUT}" \
  ...
  Rscript "${PHASE4D_DIR}/cheat27_sd_nahr_substrate.R" \
    --candidate "$cid" \
    --outdir "${RAW_OUT}" \
  ...
  Rscript "${PHASE4D_DIR}/cheat28_tandem_repeat_context.R" \
    --candidate "$cid" \
    --outdir "${RAW_OUT}" \
  ...
  Rscript "${PHASE4D_DIR}/cheat29_junction_forensics.R" \
    --candidate "$cid" \
    --outdir "${RAW_OUT}" \
  ...
  if registry_check_validation "$cid" UNCERTAIN; then
    ...
    Rscript "${PHASE4D_DIR}/cheat6_ancestry_jackknife_v934.R" \
      --candidate "$cid" \
      --outdir "${RAW_OUT}" \
