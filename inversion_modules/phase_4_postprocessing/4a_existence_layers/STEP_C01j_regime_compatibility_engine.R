...
# Registry is optional: if the loader or helpers aren't found, reg stays NULL
# and write_block_safe falls back to a JSON dump in the candidate outdir.
.try_source <- function(path) {
  if (!is.null(path) && nzchar(path) && file.exists(path)) {
    tryCatch({ source(path); TRUE }, error = function(e) {
      message("[C01j] could not source ", path, ": ", conditionMessage(e))
      FALSE
    })
  } else FALSE
}
...
reg <- if (exists("load_registry", mode = "function")) {
  tryCatch(load_registry(), error = function(e) {
    message("[C01j] load_registry() failed: ", conditionMessage(e),
...
all_memberships <- list()  # chat-13 Finding BH: C01i_b needs this at end
...
for (ci in seq_len(nrow(cand_dt))) {
  ...
  # chat-13 Finding BH: keep memberships so we can write the sidecar tsv.gz
  # at the end for C01i_b to consume.
  if (nrow(regime$memberships) > 0) {
    all_memberships[[length(all_memberships) + 1]] <- regime$memberships
  }

  # =================================================================
  # chat-13 WIRING: per-candidate regime_segments block
  # =================================================================
  # Follows the schema registries/schemas/structured_block_schemas/
  # regime_segments.schema.json. Raw memberships stay as sidecar TSV.gz
  # (too large for registry JSON); the block carries the scan_params
  # + the per-segment / per-transition arrays.
  #
  # The dominant_state is the state-string that holds the most windows
  # inside this candidate (tie-break by first occurrence). The
  # regime_has_recombinant_signal flag fires when the interval contains
  # a structured→structured transition in which the group_id
  # attribution changes — the cheap cohort-level check is at least two
  # distinct non-transition segments with different states. The
  # n_samples_with_regime_change counter comes out of stability
  # analysis (sample_type ∈ {simple_recombinant, double_crossover,
  # complex_mosaic} all count as "regime-change" for the cohort flag).
  cid <- paste0(chr, "_", iid)

  .as_block_list <- function(dt, pick_cols) {
    # Convert a data.table to a list-of-named-lists as expected by the
    # JSON schema array, picking only the fields the schema knows about.
    # Missing columns become NULL (dropped on JSON write).
  ...
  seg_fields <- c("segment_id", "state", "n_windows", "start_mb", "end_mb",
                   "mean_n_groups", "mean_thickness", "mean_separation",
                   "mean_persistence", "mean_entropy")
  trans_fields <- c("from_segment", "to_segment", "from_state", "to_state",
                     "boundary_mb", "group_change", "thickness_change",
                     "transition_type")

  # Dominant state by window-weighted mode.
  dom_state <- if (nrow(seg$segments) > 0) {
    tab <- seg$segments[, .(total = sum(n_windows)), by = state][order(-total)]
    as.character(tab$state[1])
  } else NA_character_

  # regime_has_recombinant_signal: at least one transition between two
  # different structured states, OR at least two non-transition distinct
  # states in the interval. Keep it conservative and boolean-typed.
  has_recomb <- FALSE
  if (nrow(seg$transitions) > 0) {
    structured_re <- "clean_inversion|structured_"
    has_recomb <- any(
      grepl(structured_re, seg$transitions$from_state) &
      grepl(structured_re, seg$transitions$to_state) &
      seg$transitions$from_state != seg$transitions$to_state
    )
  }

  n_samp_changed <- if (nrow(stab) > 0) {
    sum(stab$sample_type %in%
          c("simple_recombinant", "double_crossover", "complex_mosaic"))
  } else 0L

  reg_seg_block <- list(
    candidate_id = cid,
    segments = .as_block_list(seg$segments, seg_fields),
    transitions = .as_block_list(seg$transitions, trans_fields),
    n_segments = as.integer(nrow(seg$segments)),
    dominant_state = dom_state,
    regime_has_recombinant_signal = isTRUE(has_recomb),
    n_samples_with_regime_change = as.integer(n_samp_changed),
    scan_params = list(
      window_markers = as.integer(window_markers),
      step_markers   = as.integer(step_markers)
    )
  )

  cand_outdir <- file.path(outdir, "blocks", cid)
...
# chat-13 Finding BH: C01i_b's --regime_memb argument points at this file.
# The previous behaviour was to compute memberships per candidate and then
# discard them — the header at line 41 advertised this file but the writer
# was missing. The consumers (lib_recomb_combination::derive_R_from_regime)
# require columns: sample, pos_mid_mb, group_id, plus chrom for subsetting.
memb_dt <- if (length(all_memberships) > 0) {
  rbindlist(all_memberships, fill = TRUE)
} else data.table()
fwrite(memb_dt, file.path(outdir, "regime_memberships.tsv.gz"), sep = "\t")
