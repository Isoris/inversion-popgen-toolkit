...
for (ci in seq_len(nrow(cand_dt))) {
  ...
  cand_id <- if ("candidate_id" %in% names(cand)) cand$candidate_id
             else if ("snake_id" %in% names(cand)) cand$snake_id
             else ci
  ...
  # Define 5 segments
  segs <- define_segments(inv_start, inv_end)
...
if (nrow(seg_sample_dt) > 0) {
  ...
  if (nrow(cell_map_dt) > 0) {
    ...
    for (cid in unique(cell_map_dt$candidate_id)) {
      ...
      inv_s <- cand_dt[candidate_id == cid | snake_id == cid]$start_bp[1] / 1e6
      inv_e <- cand_dt[candidate_id == cid | snake_id == cid]$end_bp[1] / 1e6
