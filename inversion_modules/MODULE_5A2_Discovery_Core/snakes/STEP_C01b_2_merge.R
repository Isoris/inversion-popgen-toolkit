...
# =============================================================================
# STEP_C01b_2_merge.R  (v8.5 — fuzzy composition + landscape integration)
#
# MERGE ENGINE — reads cores from STEP_C01b, runs three merge families
# using FUZZY MAX-MIN COMPOSITION of two parallel relations:
#
#   Relation M (membership): Soft sample-band assignments transferred
#                             between cores via fuzzy composition T = R ∘ S
#   Relation G (geometric):  sim_mat continuity between core tails/heads
#
# v8.5: integrates landscape outputs from PHASE_01C:
#   - block_concordance: shared carriers between landscape blocks
#   - blue_cross_verdicts: assembly error vs real boundary
#   - boundary_catalog: boundary type (clear_hard/soft/inner/diffuse)
#   These adjust the fuzzy merge score via landscape_adjust().
#
# The three tiers differ in:
#   1. accept_threshold (how high the combined score must be)
#   2. max_gap_windows (proximity gate)
#   3. combination operator (MAX / weighted mean / MIN)
#
# Usage:
#   Rscript STEP_C01b_2_merge.R <precomp_dir> <outdir> [--tag <label>]
#     [--landscape_dir <dir>]
#
# Reads: <outdir>/snake1_cores_<chr>.rds (from core phase)
# Writes: snake_merge_*.tsv.gz, snake_candidate_regions.tsv.gz,
#         snake_merge_scores.tsv.gz (per-pair fuzzy score log)
# =============================================================================
...
BOUNDARY_HARD_PEN  <- -0.15  # clear_hard boundary between cores → penalize
...
geometric_continuity <- function(core_a_idxs, core_b_idxs, sim_mat,
                                  gap_idxs = integer(0)) {
...
compute_merge_score <- function(dt, core_a_idxs, core_b_idxs, sample_names,
...
#   1. Block concordance: do the two cores share carriers?
#   2. Blue-cross verdicts: is the gap an assembly error or real boundary?
#   3. Boundary catalog: what type of boundary sits between the cores?
# Returns: list(adjusted_score, adjustment, reason)

landscape_adjust <- function(raw_score, dt, core_a_idxs, core_b_idxs,
  ...
  a_end_bp   <- max(dt$end_bp[core_a_idxs])
  b_start_bp <- min(dt$start_bp[core_b_idxs])
  a_mid_bp   <- (min(dt$start_bp[core_a_idxs]) + a_end_bp) / 2
  b_mid_bp   <- (b_start_bp + max(dt$end_bp[core_b_idxs])) / 2

  # --- 1. Block concordance ---
  # Find which blocks the two cores fall in
  if (!is.null(block_pa_chr) && nrow(block_pa_chr) > 0) {
    a_blocks <- unique(block_pa_chr[start_bp >= min(dt$start_bp[core_a_idxs]) &
                                     end_bp <= a_end_bp &
                                     block != "bg", block])
    b_blocks <- unique(block_pa_chr[start_bp >= b_start_bp &
                                     end_bp <= max(dt$end_bp[core_b_idxs]) &
                                     block != "bg", block])
  ...
  # --- 3. Boundary catalog between cores ---
...
run_merge_fuzzy <- function(all_cores, dt, sample_names, sim_mat, chr, params,
  ...
  if (length(all_cores) == 0) return(list(regions = list(), score_log = list()))
...
run_qc <- function(merged, sim_mat) {
  ...
  for (mi in seq_along(merged)) {
    ...
    if (n_core > 0 && n_bridge / n_core > 0.5) flags <- c(flags, "overmerge")
...
if (!is.null(sample_names_snake)) message("[merge] Samples: ", length(sample_names_snake))

# Load cores
core_files <- sort(list.files(outdir, pattern = "^snake1_cores_.*\\.rds$", full.names = TRUE))
if (length(core_files) == 0) {
  all_cores_file <- file.path(outdir, "snake1_cores_all.rds")
  if (!file.exists(all_cores_file)) {
    stop("No core files found. Run STEP_C01b with --step cores first.")
  }
  all_chr_cores <- readRDS(all_cores_file)
} else {
  all_chr_cores <- list()
  for (cf in core_files) {
    obj <- readRDS(cf)
    all_chr_cores[[obj$chrom]] <- obj
  }
}
message("[merge] Core data loaded for ", length(all_chr_cores), " chromosomes")
...
snake_id <- max(0L, unlist(lapply(all_chr_cores, function(x) {
  max(0L, vapply(x$cores, function(c) c$snake_id, integer(1)))
})))

for (chr in chroms) {
  pc <- precomp_list[[chr]]
  chr_cores_obj <- all_chr_cores[[chr]]
  if (is.null(pc) || is.null(chr_cores_obj)) { message("[SKIP] ", chr); next }

  dt <- pc$dt; sim_mat <- pc$sim_mat
  n <- nrow(dt)
  all_cores <- chr_cores_obj$cores

  message("\n[merge] ======= ", chr, " =======")
  message("[merge] ", length(all_cores), " cores from core phase")
  if (length(all_cores) == 0) next
  ...
  for (mfam in MERGE_FAMILIES) {
    result <- run_merge_fuzzy(all_cores, dt, sample_names_snake, sim_mat, chr, mfam,
  ...
  reg_dt_chr <- rbindlist(all_region_rows[vapply(all_region_rows,
    function(x) is.data.table(x) && nrow(x) > 0 && x$chrom[1] == chr &&
                !is.na(x$snake_phase[1]) && x$snake_phase[1] == "merged",
    logical(1))], fill = TRUE)
  ...
  if (nrow(reg_dt_chr) > 0) {
    ...
    for (ai in seq_len(nrow(rA))) {
      ...
      all_compare_rows[[length(all_compare_rows) + 1]] <- data.table(
        chrom = chr, merge_A_id = rA$snake_id[ai],
        ...
        merge_B_ids = if (n_b > 0) paste(b_ov$snake_id, collapse = ";") else NA,
        merge_C_ids = if (n_c > 0) paste(c_ov$snake_id, collapse = ";") else NA,
        agreement = agreement
      )
    }
  ...
  # =================================================================
  # Like the core PA but for merge regions. For each window:
  # - Which merge tier (A/B/C) claimed it?
  # - What merge region ID?
  # - What inclusion status (core_original / bridge_fuzzy)?
  # - What was the fuzzy membership score for its core?
  # Also loads core-phase window states to integrate gap info.
  ...
  # Load core-phase window states for gap info
  core_states_file <- file.path(outdir, paste0("snake1_window_states_", chr, ".tsv.gz"))
  core_states <- if (file.exists(core_states_file)) fread(core_states_file) else data.table()
  ...
  for (wi in seq_len(n)) {
    ...
    # Core-phase info
    core_row <- if (nrow(core_states) > 0) core_states[global_window_id == wid] else data.table()
    core_pa <- if (nrow(core_row) > 0) core_row$pa_pattern[1] else NA_character_
    core_state <- if (nrow(core_row) > 0) core_row$collector_state[1] else NA_character_
    is_seed_like <- if (nrow(core_row) > 0) core_row$is_seed_like[1] else FALSE
    n_core_fam <- if (nrow(core_row) > 0) core_row$n_families_claimed[1] else 0L

    # Determine role in merge
    merge_role <- if (n_tiers > 0) {
      # Check if it was a bridge or original core
      statuses <- c(
      ...
      else if (any(statuses %in% c("seed", "accepted"))) "core_window"
    ...
    merge_pa_rows[[wi]] <- data.table(
      ...
      n_core_families = n_core_fam,
      is_seed_like = is_seed_like,
      # Merge phase PA
      in_merge_A = in_A, in_merge_B = in_B, in_merge_C = in_C,
      region_A = if (in_A) a_info$snake_id else NA_integer_,
      region_B = if (in_B) b_info$snake_id else NA_integer_,
      region_C = if (in_C) c_info$snake_id else NA_integer_,
...
files <- c(
  snake_merge_regions    = "snake_merge_regions.tsv.gz",
  snake_merge_hierarchy  = "snake_merge_hierarchy.tsv.gz",
  snake_merge_windows    = "snake_merge_windows.tsv.gz",
  snake_merge_qc         = "snake_merge_qc.tsv.gz",
  snake_merge_summary    = "snake_merge_summary.tsv",
  snake_merge_comparison = "snake_merge_comparison.tsv.gz",
  snake_merge_scores     = "snake_merge_scores.tsv.gz",
  snake_merge_pa         = "snake_merge_window_pa.tsv.gz"
)
  ...
  if (nrow(merge_a) > 0) {
    for (ri in seq_len(nrow(merge_a))) {
      ...
      snake_cand_rows[[scid]] <- data.table(
        candidate_id = scid, source = "snake_merge_A",
        snake_id = r$snake_id, chrom = r$chrom,
...
snake_cand_dt <- if (length(snake_cand_rows) > 0) rbindlist(snake_cand_rows, fill = TRUE) else data.table()
fwrite(snake_cand_dt, file.path(outdir, "snake_candidate_regions.tsv.gz"), sep = "\t")
message("[merge] Candidates: ", nrow(snake_cand_dt), " -> ", file.path(outdir, "snake_candidate_regions.tsv.gz"))
...
if (nrow(scores_dt) > 0) {
  ...
  g3 <- ggplot(scores_dt, aes(x = gap_windows, y = fuzzy_score, color = decision)) +
    ...
    labs(title = "Fuzzy score vs gap size",
         x = "Gap (windows between cores)", y = "Fuzzy merge score") +
    theme_minimal(base_size = 10) +
    theme(legend.position = "bottom")
...
if (nrow(reg_dt) > 0) {
  ...
  for (chr in unique(reg_dt$chrom)) {
    chr_reg <- reg_dt[chrom == chr & snake_phase == "merged"]
...
if (nrow(reg_dt) > 0 && !is.null(sample_names_snake)) {
  ...
  if (n_to_plot > 0) {
    ...
    for (pi in seq_len(n_to_plot)) {
      ...
      memb_a <- soft_band_membership(dt_chr, half_a, sample_names_snake)
      memb_b <- soft_band_membership(dt_chr, half_b, sample_names_snake)
