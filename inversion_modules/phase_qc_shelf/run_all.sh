...
: "${ANC_SCALES:=thin,dense}"
: "${Q06_PRECOMP_SCALES:=thin,dense}"
...
run_chr() {
  ...
  [[ "${SKIP_Q06_PRECOMP}" == "1" ]] || bash "${here}/STEP_Q06_precompute.sh" "${c}" "${Q06_PRECOMP_SCALES}"
  [[ "${SKIP_Q06}" == "1" ]] || Q06_PRIMARY_SCALE=dense bash "${here}/STEP_Q06_ancestry_tracks.sh" "${c}"
