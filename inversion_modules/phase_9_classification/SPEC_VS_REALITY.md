...
`cheat28_tandem_repeat_context.R` exists in `phase_8_evidence_biology/q4_mechanism/` and
...
- `phase_9_classification/compute_candidate_status.R::build_key_spec()`
  — the code source of truth.
- `phase_9_classification/characterize_candidate.R` — the per-Q
  convergence functions that consume these keys.
- `phase_9_classification/run_characterize.R` — chat-10 driver.
