...
# =============================================================================
# (renamed from run_phase4_v5_new_evidence.sh in pass 15)
...
python3 "$SCRIPT_DIR/q4_mechanism/cheat29b_assembled_junction.py" \
...
python3 "$SCRIPT_DIR/q7_existence_audit/bnd_sided_support.py" \
    --candidates "$CANDIDATES" \
    --delly_bnd_vcf "$DELLY_BND_VCF" \
    --manta_raw_vcf "$MANTA_RAW_VCF" \
    --outdir "$V5_BLOCKS" \
...
python3 "$SCRIPT_DIR/q5_age_and_origin/cross_species_bridge_v6.py" \
    --candidates "$CANDIDATES" \
    --between_species_bed "$BETWEEN_SPECIES_BED" \
    --flank_coherence_tsv "$FLANK_COHERENCE_TSV" \
    --polarized_tsv "$POLARIZED_TSV" \
...
if [[ -n "$BP_CONSENSUS_TSV" && -n "$BP_FRAGMENTS_TSV" ]]; then
    ...
    python3 "$SCRIPT_DIR/bp_bridge/bp_pipeline_bridge.py" \
