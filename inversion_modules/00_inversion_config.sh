...
# ── Code root (the clean codebase) ───────────────────────────────────────────
CODEBASE="${BASE}/inversion_codebase_v8.5"
# Legacy alias — the monolithic MODULE_5A was split into 5A1/5A2/5A3 in v8.3.
# DISCOVERYDIR now points to 5A2 (the core discovery code) for backward compat.
...
# ── Utils + bridge wiring (v9.1) ────────────────────────────────────────────
# load_bridge.R provides smap (Ind↔CGA), sample_registry, instant Q
UTILS_DIR="${CODEBASE}/utils"
export LOAD_BRIDGE="${UTILS_DIR}/load_bridge.R"
SAMPLE_MAP_R="${UTILS_DIR}/sample_map.R"
SAMPLE_REGISTRY_R="${UTILS_DIR}/sample_registry.R"
THEME_FILE="${UTILS_DIR}/theme_systems_plate.R"
...
SAMPLE_LIST="${POPSTRUCT_DIR}/list_of_samples_one_per_line_same_bamfile_list.tsv"
export PRUNED_LIST="${POPSTRUCT_DIR}/06_relatedness/pruned_samples.txt"
export RELATEDNESS_RES="${POPSTRUCT_DIR}/06_relatedness/catfish_226_relatedness.res"

# NGSadmix results (best K auto-discovered from best_seed_by_K.tsv)
NGSADMIX_DIR="${POPSTRUCT_DIR}/05_NGSadmix"
BEST_SEED_BY_K="${NGSADMIX_DIR}/best_seed_by_K.tsv"
...
# Prefix used by STEP10v2 + snake/consensus/HMM/rescue outputs.
...
# Allowed: chromosome, global, chunked_2x, chunked_3x, chunked_4x
MDS_MODE_DEFAULT="chunked_2x"
MDS_SEED_DEFAULT=42
MDS_DIMS_DEFAULT=20
Z_THRESH_DEFAULT=3

# Background sampling for chunked MDS
# chunked_2x = focal windows + 2x background windows
# chunked_4x = focal windows + 4x background windows
BACKGROUND_MULTIPLIER_DEFAULT=2

# Positional control merge defaults (CONTROL only, not primary method)
...
# Backward-compat aliases (used by MODULE_5B config_inversion_followup.R)
...
# NN-smoothed similarity scales (v8.5.3)
# Used by precomp (stored in RDS) and triangle multiscale detection.
# k=0 is the raw sim_mat (always present). Higher k = coarser.
# Precomp stores sim_mat_nn<k> for each k in NN_SIM_SCALES.
# Triangle multiscale runs detection on all scales: 0 + NN_SIM_SCALES.
export NN_SIM_SCALES="20,40,80"
...
# ---------------------------------------------------------------------------
# These are COMPARISON / CONTROL-style defaults only.
# They should not override the main snake-primary pipeline unless explicitly used.

CLASSIC_LOCALPCA_OUTLIER_Z_DEFAULT=2
CLASSIC_MAX_GAP_WINDOWS_DEFAULT=20

# Windowing note:
# The published approaches often used fixed-bp windows (e.g. 1 kb / 10 kb / 100 kb),
# whereas the current pipeline primarily uses fixed-SNP windows (e.g. 100 SNP).
# So these settings define a comparison branch, not an exact one-to-one replication.
CLASSIC_WINDOW_MODE_DEFAULT="fixed_bp"
CLASSIC_WINDOW_BP_SET_DEFAULT="1000,10000,100000"

# Classic supporting heuristics
