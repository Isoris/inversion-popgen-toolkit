...
# boundary should keep this score high (blue) as W grows; a noise peak
...
# A boundary is STABLE_BLUE if val_score stays >= this at all val_W's:
boundary_val_min       <- as.numeric(get_arg("--boundary_val_min", "0.3"))
  ...
  if (i_hi < i_lo) {
    ...
    if (length(peaks) > 0L) {
      ...
      if (boundary_validate) {
        ...
        # Classify status
        classify <- function(vmin, vslope) {
          if (!is.finite(vmin)) return("EDGE")
          if (vmin >= boundary_val_min &&
              (is.na(vslope) || vslope >= boundary_val_slope_neg))
            return("STABLE_BLUE")
          if (is.finite(vslope) && vslope <= boundary_val_slope_neg)
            return("DECAYS")
          "MARGINAL"
        }
        bdt[, validation_status := mapply(classify, val_score_min, val_slope)]
        ...
        if (n_stable_in_L1 > 0L) {
          ...
          for (k in seq_len(nrow(show))) {
            cat(sprintf("[D17L1]     %s  in %s  w=%d  %.2f Mb  trig=%.2f  val_min=%.2f  slope=%.3f\n",
                        show$boundary_idx[k],
                        show$L1_id[k],
                        show$boundary_w[k],
                        show$boundary_bp[k] / 1e6,
                        show$boundary_score[k],
                        show$val_score_min[k],
                        show$val_slope[k]))
          }
        }
      }
      ...
      for (k in seq_len(nrow(top))) {
        if ("validation_status" %in% names(top)) {
          cat(sprintf("[D17L1]     %s  w=%d  %.2f Mb  score=%.2f  status=%s  val_min=%.2f  slope=%.3f\n",
