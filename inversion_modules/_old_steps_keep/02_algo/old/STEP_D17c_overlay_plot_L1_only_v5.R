...
if (nrow(bdt) > 0L) {
  ...
  # Square geometry: lower-right corner anchored at (i, i) on the diagonal,
  # extending up-and-left. The drawn box covers rows [i-W+1, i] and
  # cols [i-W+1, i].
  brect[, sq_xmax := boundary_w                 + 0.5]
  brect[, sq_xmin := boundary_w - draw_W + 1L   - 0.5]
  brect[, sq_ymin := boundary_w                 - 0.5]
  brect[, sq_ymax := boundary_w + draw_W - 1L   + 0.5]
  # Clip to plot area
  brect <- brect[sq_xmax <= n_windows_total &
                 sq_ymax <= n_windows_total &
                 sq_xmin >= 0L & sq_ymin >= 0L]
  # Label sits just above the upper-right corner
  brect[, lab_x_b := (sq_xmin + sq_xmax) / 2]
  brect[, lab_y_b := sq_ymax + (n_windows_total * 0.012)]
  # Label content: id + Mb + trigger score + perp-ray stats + IN-L1 if so
