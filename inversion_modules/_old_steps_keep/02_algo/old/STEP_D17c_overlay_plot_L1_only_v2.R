...
label_size   <- as.numeric(get_arg("--label_size", "2.4"))
...
if (nrow(bdt) > 0L) {
  bW <- if ("boundary_W" %in% names(bdt))
          as.integer(bdt$boundary_W[1]) else 5L
  bG <- if ("boundary_offset" %in% names(bdt))
          as.integer(bdt$boundary_offset[1]) else 5L
  brect <- copy(bdt)
  # The boundary at window i lives at row range [i-W+1, i] and col range
  # [i+G+1, i+G+W] in the upper triangle.
  brect[, sq_xmin := boundary_w - bW + 1L - 0.5]
  brect[, sq_xmax := boundary_w           + 0.5]
  brect[, sq_ymin := boundary_w + bG + 1L - 0.5]
  brect[, sq_ymax := boundary_w + bG + bW - 0.5 + 0.5]
  # Clip to plot area
  brect <- brect[sq_xmax <= n_windows_total &
                 sq_ymax <= n_windows_total &
                 sq_xmin >= 0L & sq_ymin >= 0L]
  brect[, lab_x_b := (sq_xmin + sq_xmax) / 2]
  brect[, lab_y_b := (sq_ymin + sq_ymax) / 2 +
                     (n_windows_total * 0.012)]
  # Label includes status + L1 membership when present
  has_val_min <- "val_score_min" %in% names(brect)
  has_inside  <- "inside_L1"     %in% names(brect)
  brect[, label_text_b := {
    base <- sprintf("%s  %.2f Mb  s=%.1f",
                    boundary_idx, boundary_bp / 1e6, boundary_score)
    if (has_val_min) base <- sprintf("%s  vmin=%.2f", base, val_score_min)
