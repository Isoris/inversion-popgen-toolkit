...
#
# Phase 4 / 4c — group validation
#
# Codebase:     inversion_modules v10.1.1 / script v9.3.4 + integrated patches
# Upstream:     phase 4b (C01i decompose / multi_recomb / nested_comp / seal)
#               writes q6_group_validation=UNCERTAIN + registered groups.
# Downstream:   phase 4d group-dependent cheats gate on q6_group_validation
#               >= SUPPORTED
# Formerly:     STEP_C01f_hypothesis_tests_wired_6_9_12_23_v934_registry.R
#
# Base logic is v9.3.4 (T1-T11 hypothesis tests + verdict synthesis).
#
# Integrated patches (see ../patches/ for originals and application order):
#   [01] comp_from_registry.R     — read groups from sample_registry (C01i output)
#                                   with fallback to PC1 k-means from
#                                   triangle_sample_composition.tsv.gz
#   [02] promotion_cap.R          — respect q6_validation_promotion_cap from 4b.4
#                                   (composite_flag=likely_composite → cap=UNCERTAIN)
#   [03] jackknife_semantics.R    — four-way jackknife classification; returns
#                                   list(level, quality_flags, family_linkage);
#                                   single_family_fragile is REAL, not SUSPECT
#
# Hypothesis tests:
...
#   T4: Inner vs outer sample composition overlap (H2 vs H3)  [per-subregion km]
#   T5: Anchor stability across sub-regions (H2)               [per-subregion km]
#   T6: Regime change at inner/outer boundary (H3)             [per-subregion km]
#   T7: Within-carrier-only substructure (H4)
#   T8: Clair3 indel genotype concordance (Cheat 9, H5)
#   T9: Ancestry jackknife (Cheat 6)  — feeds validation level
#   T10: Theta het prior (Cheat 12)
#   T11: Extended boundary suppression (Cheat 23)
#
# v9.3.4 changes (original, preserved):
#   - T9 + T10 WIRED INTO run_decision_tree() verdict synthesis.
#   - T9 ancestry jackknife: Engine B (load_bridge.R) get_region_stats(Fst)
#     if available, falls back to sim_mat method.
#   - T10 theta het concordance: integrated as independent confirmation layer.
...
#   hypothesis_verdicts.tsv         -- per-candidate verdict table (now with
#                                       group_validation_before/after +
#                                       quality_flags + family_linkage columns)
...
# =============================================================================
# Patch [01] + [02] + [03] — registry-aware group building + cap-aware,
# jackknife-semantics-aware compute_group_validation.
# Inserted here (between config and tests) per patch (A) instruction.
# =============================================================================

# ─── comp_from_registry ───────────────────────────────────────────────────────
# Build the `comp` data.table (columns: sample, band, source) from C01i's
# registered groups for this candidate. Returns NULL if groups are absent or
# insufficient, so the caller can fall back to PC1 k-means.
#
# Band mapping convention (matches legacy PC1-ordered k-means output):
#   band1 = HOM_REF    (lowest PC1)
#   band2 = HET        (middle PC1)
#   band3 = HOM_INV    (highest PC1)
# RECOMBINANT samples are EXCLUDED — they violate the "one band per sample"
# assumption of T1/T2/T7/T8/T10. Class switches inside the interval; averaging
# pollutes the within-band statistic. Handled separately by 4b.2 multi_recomb.
comp_from_registry <- function(cid, sample_names,
  ...
  # Intersect with available sample names (e.g., after kin-pruning)
  ...
  attr(comp, "source") <- "C01i_registry"
  comp
}
...
# Four-way jackknife classification (v10.1.1) + promotion cap (v10.1) +
# base v10 Fisher/T8 promotion rules. Returns a list:
#   $level           UNCERTAIN | SUPPORTED | VALIDATED | SUSPECT
#   $quality_flags   character vector (or "normal")
#   $family_linkage  multi_family | few_family | single_family |
#                    pca_family_confounded | uncertain | unknown
#
# Pure function — no registry writes. Caller stores the returned list fields
# on the verdict row; the schema's keys_extracted flattens them.
compute_group_validation <- function(current_level,
  ...
  internal_cap <- NA_character_  # derived from jackknife

  # ── Four-way jackknife classification ──
  ...
  if (jk == "robust_multi_family") {
    ...
    internal_cap <- "SUPPORTED"  # real, but lineage-restricted
  } else if (jk == "fragile" || jk == "pca_family_driven") {
    # PCA groups track ancestry, not arrangement — genuine demotion
  ...
  # Fallback when verdict missing but raw delta is high
  ...
  for (cap in c(promotion_cap, internal_cap)) {
    if (!is.na(cap) && nzchar(cap) && cap %in% names(rank_map)) {
      if (rank_map[best] > rank_map[cap]) {
        message("[C01f] cap ", cap, " overrides ", best)
        best <- cap
      }
    }
  }
...
for (ci in seq_len(nrow(cand_dt))) {
  ...
  cid <- paste0(chr, "_", iid)          # candidate_id — used by registry lookup
  ...
  # ── Patch (C) — compute group_validation from t8/t9/t10 + registry cap ──
  current_level <- tryCatch({
    if (exists("reg") && !is.null(reg$evidence) && !is.null(reg$evidence$get_evidence)) {
      ev <- reg$evidence$get_evidence(cid, "q6_group_validation")
      if (!is.null(ev) && is.data.frame(ev) && nrow(ev) > 0) as.character(ev$value[1]) else "UNCERTAIN"
    } else "UNCERTAIN"
  }, error = function(e) "UNCERTAIN")

  promotion_cap <- tryCatch({
    if (exists("reg") && !is.null(reg$evidence) && !is.null(reg$evidence$get_evidence)) {
      cap_ev <- reg$evidence$get_evidence(cid, "q6_validation_promotion_cap")
      if (!is.null(cap_ev) && is.data.frame(cap_ev) && nrow(cap_ev) > 0) as.character(cap_ev$value[1]) else NA_character_
    } else NA_character_
  ...
  message("[C01f] ", cid, ": group validation ", current_level, " → ", gv$level,
          " [family_linkage=", gv$family_linkage, "]")
  ...
  all_verdict_rows[[length(all_verdict_rows)+1]] <- data.table(
    ...
    # ── patch (C): group-validation outcome from compute_group_validation() ──
    group_validation_before = current_level,
    group_validation_after  = gv$level,
    quality_flags           = paste(gv$quality_flags, collapse = ","),
    family_linkage          = gv$family_linkage,
    promotion_cap_applied   = if (!is.na(promotion_cap)) promotion_cap else "",
    comp_source             = attr(iv_comp, "source") %||% "unknown",
    # ── decision-tree step columns (unchanged) ──
