...
# =============================================================================
# STEP_C00_build_flashlight.R — Build SV-derived prior ("flashlight") per chr
#   (v9.3.2 — Cheat 8 BND triangulation wired)
#
# Reads DELLY2 + Manta INV/DEL/BND catalogs, extracts per-sample genotypes,
# identifies het-DELs at inversion breakpoints, triangulates inversions from
# paired BND calls, and packages everything into a per-chromosome RDS.
#
# IMPLEMENTS:
#   Cheat 1: DELLY/Manta INV genotypes as anchor samples
#   Cheat 2: Het-DELs at inversion breakpoints as arrangement markers
#   (Cheat 3: het-DEL haplotype separation — deferred to decomposition)
#   Cheat 8: BND paired breakpoint triangulation — extends anchor samples
#            with inversions that DELLY classified as BND instead of INV
#
# Usage:
#   Rscript STEP_C00_build_flashlight.R [--chr C_gar_LG01]
#   Rscript STEP_C00_build_flashlight.R               # all chromosomes
#
# Requires:
#   - DELLY2/Manta INV VCFs with per-sample GT fields
#   - DELLY2/Manta DEL VCFs for het-DEL extraction
#   - Population confidence TSVs (from annotate_population_confidence.sh)
#   - 00_inversion_config.sh sourced by launcher (provides BASE, SAMPLES_IND)
#
# Output:
#   <outdir>/flashlight/sv_flashlight_<chr>.rds  — per-chromosome
#   <outdir>/flashlight/flashlight_summary.tsv   — genome-wide summary
# =============================================================================
...
if (nzchar(.bridge_file) && file.exists(.bridge_file)) {
  tryCatch({ source(.bridge_file); .bridge_available <- TRUE
    message("[flashlight] load_bridge.R sourced")
  }, error = function(e) message("[flashlight] load_bridge.R failed: ", conditionMessage(e)))
}
...
# Cheat 8: BND VCF for triangulation
...
FLASH_DIR <- file.path(INVDIR, "06_mds_candidates/snake_regions_multiscale/flashlight")
dir.create(FLASH_DIR, recursive = TRUE, showWarnings = FALSE)

# Parameters
BP_MATCH_WINDOW <- 50000L   # ±50 kb: het-DEL within this of INV breakpoint = Cheat 2
MIN_DEL_SIZE    <- 5000L    # minimum het-DEL size for Cheat 2/3
...
if (!file.exists(SAMPLES_IND)) stop("[flashlight] SAMPLES_IND not found: ", SAMPLES_IND)
sample_ids <- trimws(readLines(SAMPLES_IND))
sample_ids <- sample_ids[nzchar(sample_ids)]
n_samples <- length(sample_ids)
message("[flashlight] Samples: ", n_samples)

# =============================================================================
# CHEAT 8: BND PAIRED BREAKPOINT TRIANGULATION
# =============================================================================
...
parse_bnd_pairs <- function(bnd_vcf_path) {
  if (!file.exists(bnd_vcf_path)) {
    message("[cheat8] BND VCF not found: ", bnd_vcf_path, " — skipping")
    return(data.table())
  }
  ...
  if (is.null(vcf_samples) || length(vcf_samples) == 0) {
    message("[cheat8] Cannot read sample names from BND VCF")
    return(data.table())
  }
  ...
  if (is.null(raw) || nrow(raw) == 0) {
    message("[cheat8] No BND records parsed")
    return(data.table())
  }

  n_samp <- length(vcf_samples)
  if (ncol(raw) < 6 + n_samp) {
    message("[cheat8] Column count mismatch — expected ", 6 + n_samp, " got ", ncol(raw))
    return(data.table())
  }
  ...
  if (nrow(raw) == 0) {
    message("[cheat8] No same-chr inversion-oriented BNDs found")
    return(data.table())
  }
  message("[cheat8] Same-chr inv BNDs: ", nrow(raw),
...
message("[cheat8] Total BND-triangulated inversions: ", nrow(bnd_pairs_all))
...
parse_sv_vcf <- function(vcf_path, caller_name, svtype_filter = NULL,
  ...
  if (!file.exists(vcf_path)) {
    message("[flashlight] VCF not found: ", vcf_path)
    return(data.table())
  }

  message("[flashlight] Parsing ", caller_name, ": ", vcf_path)
  ...
  message("[flashlight]   Total records: ", n_total, " → ", n_pass, " pass filters")
...
map_vcf_to_cga <- function(vcf_gt_list, vcf_samples, cga_ids) {
  ...
  message("[flashlight] WARNING: cannot map VCF samples to CGA IDs")
...
message("\n[flashlight] ═══ CHEAT 1: Loading INV genotypes ═══")
...
message("[flashlight] Total INV calls: ", nrow(all_inv),
        " (DELLY: ", nrow(delly_inv), ", Manta: ", nrow(manta_inv), ")")

message("\n[flashlight] ═══ CHEAT 2: Loading het-DELs ═══")
...
message("[flashlight] Het-DELs ≥", MIN_DEL_SIZE / 1000, " kb: ", nrow(het_dels))
...
if (file.exists(inv_conf_file)) {
  ...
  if (nrow(pop_conf) > 0) {
    message("[flashlight] Population confidence: ", nrow(pop_conf), " INV records")
  }
}

# =============================================================================
# BUILD PER-CHROMOSOME FLASHLIGHT
# =============================================================================

chroms <- sort(unique(c(all_inv$chrom, het_dels$chrom)))
if (!is.null(chr_filter)) chroms <- intersect(chroms, chr_filter)
message("\n[flashlight] Building flashlight for ", length(chroms), " chromosomes")
...
for (chr in chroms) {
  ...
  # ── Cheat 1: INV genotypes for this chromosome ──
  chr_inv <- all_inv[chrom == chr]
  if (nrow(chr_inv) > 0) {
    gt_matrix <- build_genotype_matrix(chr_inv, sample_ids)
    message("[flashlight]   INV calls: ", nrow(chr_inv),
  ...
  # ── Cheat 2: Het-DELs at INV breakpoints ──
  ...
  if (nrow(breakpoint_dels) > 0 && ncol(gt_matrix) > 0) {
    for (inv_id_check in unique(breakpoint_dels$near_inv_id)) {
      ...
      # Cheat 2 predicts: het-DEL carriers should be HOM_REF or HET (not HOM_INV)
      # Because het-DEL at breakpoint means reference arrangement on ≥1 haplotype
      cheat2_works <- frac_ref_het >= 0.7

      cheat2_verification[[inv_id_check]] <- data.table(
        inv_id = inv_id_check,
        n_del_carriers = length(carrier_gt),
        n_ref_or_het = n_ref_het,
        n_hom_inv = n_hom_inv,
        frac_ref_het = round(frac_ref_het, 3),
        cheat2_concordant = cheat2_works
      )

      if (cheat2_works) {
        message("[flashlight]   Cheat 2 CONFIRMED for ", inv_id_check,
  ...
  cheat2_dt <- if (length(cheat2_verification) > 0) {
    rbindlist(cheat2_verification)
  } else {
    data.table(inv_id = character(), n_del_carriers = integer(),
               n_ref_or_het = integer(), n_hom_inv = integer(),
               frac_ref_het = numeric(), cheat2_concordant = logical())
  }

  # ── Het-DELs INSIDE inversions (for Cheat 3 — stored, used by decomposition) ──
  ...
  if (nrow(internal_dels) > 0) {
    message("[flashlight]   Internal het-DELs: ", nrow(internal_dels),
  ...
  if (ncol(gt_matrix) > 0) {
    for (inv_col in colnames(gt_matrix)) {
      ...
      for (si in seq_len(n_samples)) {
        ...
        # Check Cheat 2: does this sample have het-DEL at this INV's breakpoints?
        has_bp_del <- FALSE
        if (nrow(breakpoint_dels) > 0) {
        ...
        # Check Cheat 3: does this sample have het-DEL inside this INV?
        ...
        # Evidence count
        n_ev <- sum(c(
          gt_val != "MISSING",           # Cheat 1: has SV genotype
          has_bp_del,                     # Cheat 2: het-DEL at breakpoint
          has_internal_del               # Cheat 3: het-DEL inside
        ))
  ...
  flashlight <- list(
    ...
    # Cheat 1: INV calls with per-sample genotypes
    ...
    # Cheat 2: het-DELs at breakpoints
    breakpoint_dels = breakpoint_dels,
    cheat2_verification = cheat2_dt,

    # Cheat 3: het-DELs inside inversions (for decomposition)
    internal_dels = internal_dels,

    # Cheat 8: BND-triangulated inversions (extend anchor samples)
  ...
  rds_out <- file.path(FLASH_DIR, paste0("sv_flashlight_", chr, ".rds"))
  saveRDS(flashlight, rds_out)
  message("[flashlight]   → ", rds_out)
  ...
  summary_rows[[length(summary_rows) + 1L]] <- data.table(
    ...
    n_cheat2_confirmed = sum(cheat2_dt$cheat2_concordant, na.rm = TRUE),
...
f_summ <- file.path(FLASH_DIR, "flashlight_summary.tsv")
fwrite(summary_dt, f_summ, sep = "\t")

message("\n[flashlight] ═══ GENOME-WIDE SUMMARY ═══")
if (nrow(summary_dt) > 0) {
  ...
  message("  Cheat 2 confirmed:    ", sum(summary_dt$n_cheat2_confirmed))
  message("  Cheat 8 BND triang:  ", sum(summary_dt$n_bnd_triangulated))
...
message("\n[flashlight] Output: ", FLASH_DIR)
message("[flashlight] Summary: ", f_summ)
...
tryCatch({
  if (.bridge_available && exists("reg") && !is.null(reg) && !is.null(reg$add_evidence)) {
    ...
    # happens later when C01d reads the flashlight and scores D7.
    message("[flashlight] Registry: SV data available for ", nrow(summary_dt), " chromosomes")
  }
}, error = function(e) message("[flashlight] Registry note: ", conditionMessage(e)))
