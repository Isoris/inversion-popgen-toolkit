...
# =============================================================================
# STEP_C01b_1_cores.R  (v9.3.2 — morphology + Cheat 7 + Cheat 26)
#
...
#
# v9.3.2 changes:
#   - Morphology soft damage modifiers: flat_inv_score reduces damage
#     (snake traverses coherent plateau), fragmentation_score increases
#     damage (snake entering messy territory), spiky_inv_score boosts
#     seed priority for compact candidates.
#   - Cheat 7 adaptive_seed: if beta_pval column exists in precomp dt,
#     use adaptive_seed=TRUE as an additional seed gate (OR with z/inv).
#   - Cheat 26 kin-pruned retention: after all cores are found, optionally
#     test each core against kin-pruned sample list. Cores where the signal
#     collapses get flagged (NOT killed — see hatchery founder caveat).
#
...
if (nzchar(.bridge_file) && file.exists(.bridge_file)) {
  tryCatch({ source(.bridge_file); .bridge_available <- TRUE
    message("[cores] load_bridge.R sourced")
  }, error = function(e) message("[cores] load_bridge.R failed: ", conditionMessage(e)))
}
...
run_core_family <- function(dt, sim_mat, mds_mat, chr, snake_id_start,
  ...
  has_adaptive <- "adaptive_seed" %in% names(dt)  # Cheat 7
  has_spiky <- "spiky_inv_score" %in% names(dt)   # v9 morphology

  # Seed eligibility: z-score OR inv_likeness OR Cheat 7 adaptive_seed
  ...
  if (has_spiky) {
    ...
    # be seeded early so they're not swallowed by larger greedy snakes.
...
for (chr in chroms) {
  ...
  for (fam in CORE_FAMILIES) {
    ...
    for (reg in result$regions) {
      ...
      all_region_rows[[length(all_region_rows) + 1]] <- data.table(
        ...
        cheat26_status = reg$cheat26_status %||% NA_character_,
        cheat26_retention = reg$cheat26_retention %||% NA_real_
      )
    }
  }
  ...
  # ═══════════════════════════════════════════════════════════════════
  # CHEAT 26: KIN-PRUNED RETENTION TEST (v9.3.2)
  # ═══════════════════════════════════════════════════════════════════
  # For each core, test if the sim_mat block contrast survives after
  ...
  if (!is.null(pruned_file) && file.exists(pruned_file)) {
    ...
    if (!is.null(pruned_ind_names)) {
      message("[cheat26] Pruned samples loaded: ", length(pruned_ind_names))
    }
  }

  if (!is.null(pruned_ind_names) && length(pruned_ind_names) >= 15 && length(all_cores) > 0) {
    message("[cheat26] Testing kin-pruned retention for ", length(all_cores), " cores...")
    ...
    if (length(avail_pruned) >= 15) {
      for (ci in seq_along(all_cores)) {
        reg <- all_cores[[ci]]
        idxs <- reg$idxs
        if (length(idxs) < 5) { reg$cheat26_status <- "too_small"; next }
        ...
        if (length(win_in) >= 3 && length(out_in) >= 3) {
          ...
          reg$cheat26_retention <- round(retention, 3)
          reg$cheat26_status <- if (!is.finite(retention)) "cannot_compute"
            else if (retention > 0.7) "persists"
            else if (retention > 0.4) "weakened"
            else if (retention > 0.1) "mostly_lost"
            else "collapsed"
        } else {
          reg$cheat26_status <- "insufficient_pruned"
          reg$cheat26_retention <- NA_real_
        }

        all_cores[[ci]] <- reg
        if (ci <= 5 || ci %% 20 == 0) {
          message("  core ", ci, " (", reg$core_family, " snake ", reg$snake_id,
                  "): cheat26=", reg$cheat26_status,
                  " retention=", reg$cheat26_retention)
        }
      }

      # Summary
      c26_tab <- table(vapply(all_cores, function(r) r$cheat26_status %||% "untested", character(1)))
      message("[cheat26] Results: ", paste(names(c26_tab), c26_tab, sep = "=", collapse = " "))
    } else {
      message("[cheat26] Too few pruned PC columns (", length(avail_pruned), ") — skipping")
    }
  } else if (is.null(pruned_file)) {
    message("[cheat26] No --pruned_samples provided — skipping retention test")
  }
