...
# =============================================================================
# STEP_C01a_snake1_precompute.R  (v9.3.4 — Engine B Fst for Cheat 5)
#
...
#   - Seed NN distances
#   - Inversion-likeness scores:
#       inv_likeness = 0.45 × het + 0.30 × trimodality + 0.25 × band_discreteness
#       (PVE1/structure_likeness REMOVED — was circular)
#   - Beta adaptive threshold (Cheat 7): per-chromosome Beta(α,β) fit on
#     inv_likeness → p-value per window → adaptive_seed flag replaces
#     fixed threshold for snake seed selection
#   - Family likeness (diagnostic annotation, NOT a gate)
#   - Cheat 5 Q-stamps: per-window per-band dominant Q component, fraction,
#     cross-band Q entropy (diagnostic, NOT a gate)
#   - Dosage het rate (independent from PCA, second-pass upgrade)
#
# CHANGES v9.3.1 → v9.3.2:
#   - Cheat 5 Q-stamp columns added (8 new columns per window):
#     band1_dom_Q, band1_dom_Q_frac, band2_dom_Q, band2_dom_Q_frac,
#     band3_dom_Q, band3_dom_Q_frac, bands_same_Q, band_Q_entropy
#   - PVE1 computation fully removed (was still computing in v9.3.1)
#
...
if (nzchar(.bridge_file) && file.exists(.bridge_file)) {
  tryCatch({
    ...
    message("[PRECOMP] load_bridge.R sourced — Engine B available for Cheat 5 Fst")
  }, error = function(e) {
    message("[PRECOMP] WARNING: load_bridge.R failed: ", conditionMessage(e))
  })
} else {
  message("[PRECOMP] load_bridge.R not found — Cheat 5 Fst scan will be skipped (ANOVA family_likeness only)")
}

args <- commandArgs(trailingOnly = TRUE)
if (length(args) < 2) {
  stop("Usage: Rscript STEP_C01a_snake1_precompute.R <step10_outprefix> <outdir> [--qmatrix <qopt>] [--relatedness <pairs>] [--mode hatchery|wild]")
}
...
while (i <= length(args)) {
  ...
  else if (a == "--flashlight_dir" && i < length(args)) { flashlight_dir <- args[i+1]; i <- i+2 }
...
compute_inv_likeness_all <- function(per_chr, chroms) {
  ...
  for (chr in chroms) {
    ...
    for (wi in seq_len(nrow(dt))) {
      ...
      #   Per-chromosome calibration via Beta adaptive threshold (Cheat 7)
      ...
      # ═══════════════════════════════════════════════════════════════
      # CHEAT 5 Q-STAMP: per-band ancestry composition (v9.3.2)
      # ═══════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════
# CHEAT 7: Beta adaptive threshold (integrated v9.3.1)
# ═══════════════════════════════════════════════════════════════════
...
message("[PRECOMP] ── CHEAT 7: Beta adaptive threshold ──")
...
# ═══════════════════════════════════════════════════════════════════
# CHEAT 5: REAL Fst SCAN via Engine B dispatcher (v9.3.4)
# ═══════════════════════════════════════════════════════════════════
...
if (.bridge_available && exists("get_region_stats", mode = "function") &&
  ...
  message("[PRECOMP] ── CHEAT 5: Real Fst scan (Engine B) ──")
  ...
  # Initialize columns
  inv_like_dt[, `:=`(
    cheat5_fst_pc1 = NA_real_,
    cheat5_fst_q_best = NA_real_,
    cheat5_fst_q_best_k = NA_integer_,
    cheat5_family_fst_ratio = NA_real_
  )]
  ...
  for (chr_i in unique(inv_like_dt$chrom)) {
    ...
    for (si in scan_idx) {
      ...
      inv_like_dt[wi, `:=`(
        cheat5_fst_pc1 = round(fst_pc1 %||% NA_real_, 4),
        cheat5_fst_q_best = round(fst_q_best %||% NA_real_, 4),
        cheat5_fst_q_best_k = as.integer(fst_q_best_k %||% NA_integer_),
        cheat5_family_fst_ratio = round(ratio %||% NA_real_, 4)
      )]
      n_done <- n_done + 1L
    }
    ...
    scanned_valid <- scanned[!is.na(inv_like_dt$cheat5_fst_pc1[scanned])]
    if (length(scanned_valid) > 0) {
      for (col in c("cheat5_fst_pc1", "cheat5_fst_q_best",
                     "cheat5_fst_q_best_k", "cheat5_family_fst_ratio")) {
  ...
  n_ratio <- sum(is.finite(inv_like_dt$cheat5_family_fst_ratio))
  n_inversion_like <- sum(inv_like_dt$cheat5_family_fst_ratio < 0.3, na.rm = TRUE)
  n_family_like <- sum(inv_like_dt$cheat5_family_fst_ratio > 0.7, na.rm = TRUE)
  message("[PRECOMP] Cheat 5 Fst scan: ", n_ratio, " windows with ratio, ",
          n_inversion_like, " inversion-like (ratio<0.3), ",
          n_family_like, " family-like (ratio>0.7)")
} else {
  message("[PRECOMP] Cheat 5 Fst scan skipped (need Engine B + Q matrix)")
  # Ensure columns exist even when skipped
  if (!"cheat5_fst_pc1" %in% names(inv_like_dt)) {
    inv_like_dt[, `:=`(
      cheat5_fst_pc1 = NA_real_,
      cheat5_fst_q_best = NA_real_,
      cheat5_fst_q_best_k = NA_integer_,
...
# ═══════════════════════════════════════════════════════════════════
# CHEATS 1-3: SV FLASHLIGHT ANNOTATION (integrated v9.3.1)
# ═══════════════════════════════════════════════════════════════════
# Last annotation step — stamps SV columns onto inv_like_dt.
# Reads per-chromosome flashlight RDS built by STEP_C00_build_flashlight.R.
...
if (!is.null(flashlight_dir) && dir.exists(flashlight_dir)) {
  message("[PRECOMP] ── CHEATS 1-3: SV Flashlight annotation ──")
  n_annotated <- 0L

  for (chr in unique(inv_like_dt$chrom)) {
    fl_file <- file.path(flashlight_dir, paste0("sv_flashlight_", chr, ".rds"))
    if (!file.exists(fl_file)) {
      fl_file <- file.path(flashlight_dir, paste0(chr, "_flashlight.rds"))
    }
    ...
    # Cheat 8: BND-triangulated inversions (extend inv_calls with BND pairs)
    ...
    if (!is.null(bnd_tri) && nrow(bnd_tri) > 0) {
      ...
      message("  ", chr, ": Cheat 8 added ", nrow(bnd_tri), " BND-triangulated inversions")
    }
    ...
    for (wi in chr_idx) {
      ...
      # Cheat 3: Internal het-DELs (hemizygous segments)
  ...
  message("[PRECOMP] No --flashlight_dir provided — SV annotation columns will be 0/NA")
}
