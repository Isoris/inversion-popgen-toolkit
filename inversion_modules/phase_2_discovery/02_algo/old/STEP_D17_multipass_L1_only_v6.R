#!/usr/bin/env Rscript
# =============================================================================
# STEP_D17_multipass_L1_only_v6.R
#
# v6 changes vs v5:
#   - NEW: growing-W cross-block validator (--boundary_grow_W).
#     For each peak position i, the validator rescores the upper-triangle
#     cross-block Z[(i-W+1):i, (i+G+1):(i+G+W)] at multiple W sizes, e.g.
#     W = 5, 20, 50, 100, capturing the signed median Z at each scale.
#
#     A REAL boundary's cross-block stays blue or drifts toward 0 across
#     all W; the median Z never becomes positive — the two flanking
#     inversions are genuinely uncorrelated even at long range.
#
#     A FAKE boundary (a small blue cross sitting *inside* a single
#     inversion) shows blue cross-block at small W but, as W grows, the
#     cross-block reaches into the surrounding same-inversion red bulk on
#     both sides; the cross-block's median Z drifts from blue/null to
#     slightly positive or positive. This is the discriminator.
#
#     New columns in the boundary TSV:
#       cross_z_W{w}   : median Z of the cross-block at scale W=w
#                        (one column per --boundary_grow_W value)
#       grow_max_z     : max over those values (largest positive drift)
#       grow_status    : REAL | FAKE | EDGE based on grow_max_z thresholds
#
#     The legacy perp-ray validator still runs (for diagnostics) but the
#     overlay's classification now uses validation_status which reflects
#     the growing-W decision when --boundary_validator_mode=grow.
#
#   - validation_status semantics extended:
#       STABLE_BLUE   real boundary  (kept for back-compat with overlay)
#       DECAYS        fake boundary  (interior cross of a single inversion)
#       MARGINAL      in between
#       EDGE          too close to chr edge to validate at largest W
#
...
#
#   At each peak i, for each W in --boundary_grow_W (default 5,20,50,100):
...
boundary_grow_W_str            <- get_arg("--boundary_grow_W", "5,20,50,100")
boundary_grow_W                <- as.integer(strsplit(boundary_grow_W_str,
                                                      ",")[[1]])
boundary_grow_W                <- sort(unique(boundary_grow_W[
  is.finite(boundary_grow_W) & boundary_grow_W >= 2L]))
...
if (boundary_scan) {
  ...
  if (boundary_validator_mode %in% c("grow", "both")) {
    cat("  grow_W=", paste(boundary_grow_W, collapse = ","),
        "  real_max=", boundary_grow_real_max,
        "  fake_min=", boundary_grow_fake_min,
        "  min_largest_W=", boundary_grow_min_largest_W, sep = "")
  }
  cat("\n")
}
    ...
    if (length(peaks) > 0L) {
      ...
      if (boundary_validator_mode %in% c("grow", "both") &&
        ...
        classify_grow <- function(gmax, glargest) {
          ...
          if (gmax <= boundary_grow_real_max)          return("REAL")
          if (gmax >= boundary_grow_fake_min)          return("FAKE")
          "MARGINAL"
        }
      ...
      if (boundary_validate || boundary_validator_mode %in% c("grow", "both")) {
        ...
        if (n_stable_in_L1 > 0L) {
          ...
          for (k in seq_len(nrow(show))) {
            line <- sprintf(
              "[D17L1]     %s  in %s  w=%d  %.2f Mb  trig=%.2f",
              show$boundary_idx[k], show$L1_id[k], show$boundary_w[k],
              show$boundary_bp[k] / 1e6, show$boundary_score[k])
            if ("grow_max_z" %in% names(show)) {
              line <- sprintf("%s  grow_max_z=%+.2f", line, show$grow_max_z[k])
            }
            if ("right_frac_blue" %in% names(show)) {
              line <- sprintf("%s  rfb=%.2f lfb=%.2f rmz=%+.2f lmz=%+.2f",
                              line, show$right_frac_blue[k],
                              show$left_frac_blue[k],
                              show$right_max_z[k], show$left_max_z[k])
            }
            cat(line, "\n", sep = "")
          }
        }
