#!/usr/bin/env Rscript
# =============================================================================
# STEP_D17c_overlay_plot_L1_only.R
#
...
boundary_lw    <- as.numeric(get_arg("--boundary_lw", "0.7"))
...
if (nrow(bdt) > 0L) {
  # Trigger W (used as fallback when no survival_W column is present)
  ...
  brect <- copy(bdt)
  # Per-row drawn square size:
  #   if survival_W column exists and is positive, use it (so STABLE
  #   boundaries draw bigger than DECAYS ones — directly visualizing
  #   how far the blueness extends).
  #   Otherwise fall back to the trigger W.
  if ("survival_W" %in% names(brect)) {
    brect[, draw_W := pmax(survival_W, bW_trig)]
    brect[!is.finite(draw_W) | draw_W < 1L, draw_W := bW_trig]
  } else {
    brect[, draw_W := bW_trig]
  }
  # Diagonal-anchored square ending at (i, i) of size draw_W. The
  # upper-triangle drawn square has rows [i-W+1, i] and cols [i-W+1, i],
  # but we draw it as a square at that location (geom_rect treats it as
  # a 2D rectangle in the plot — so its visible footprint includes the
  # lower-triangle mirror, which is fine).
  brect[, sq_xmin := boundary_w - draw_W + 1L - 0.5]
  brect[, sq_xmax := boundary_w                 + 0.5]
  brect[, sq_ymin := boundary_w - draw_W + 1L - 0.5]
  brect[, sq_ymax := boundary_w                 + 0.5]
  # Clip to plot area
  brect <- brect[sq_xmax <= n_windows_total &
                 sq_ymax <= n_windows_total &
                 sq_xmin >= 0L & sq_ymin >= 0L]
  # Label sits just above the upper-right corner (in the upper triangle)
  brect[, lab_x_b := (sq_xmin + sq_xmax) / 2]
  brect[, lab_y_b := sq_ymax + (n_windows_total * 0.012)]
  # Label content: id + Mb + trigger score + survival_W + IN-L1 if so
  has_surv   <- "survival_W" %in% names(brect)
  has_inside <- "inside_L1"  %in% names(brect)
  brect[, label_text_b := {
    base <- sprintf("%s  %.2f Mb  s=%.1f",
                    boundary_idx, boundary_bp / 1e6, boundary_score)
    if (has_surv)    base <- sprintf("%s  surv=%d", base, survival_W)
