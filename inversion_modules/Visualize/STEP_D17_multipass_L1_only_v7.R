#!/usr/bin/env Rscript
# =============================================================================
# STEP_D17_multipass_L1_only_v7.R
#
# Boundary-driven L1 envelope discovery on a single chromosome.
#
# What this script does:
#   1. Loads precomp window coords + sim_mat.
#   2. Slides a small WxW upper-triangle cross-block at offset G along the
#      diagonal. At each position i:
#        cross = Z[(i-W+1):i, (i+G+1):(i+G+W)]
#        boundary_score[i] = -median(cross)
#      Bluer cross => higher score.
#   3. 1D peak detection on boundary_score with --boundary_min_dist
#      neighbourhood and --boundary_score_min floor.
#   4. Validates each peak with two independent tests:
#        (a) growing-W cross-block z (default validator):
#              re-evaluates the cross-block at multiple W sizes and tracks
#              grow_max_z = max over W of median(z). REAL boundaries stay
#              blue at all W; FAKE boundaries (small blue cross inside one
#              inversion) drift toward positive z as the cross-block reaches
#              into the surrounding same-inversion red bulk.
#        (b) perpendicular rays: 1D z values along Z[i, i+d] and Z[i-d, i].
#              REAL boundaries stay blue along both rays. (Diagnostic.)
#      Adaptive thresholds: real_max = min(q_real(grow_max_z),
#      real_max_ceiling); fake_min = max(q_fake(grow_max_z),
#      fake_min_floor). Falls back to fixed thresholds when too few peaks.
#   5. Partitions the chromosome into L1 segments using STABLE_BLUE peaks
#      as cut points. The partition replaces the legacy envelope-discovery
#      catalogue. 1-window segments (caused by adjacent-tie peaks) are
#      merged into the more-similar neighbour by mean(sim_mat[w, neighbor]).
#
# Outputs:
#   <chr>_d17L1_boundaries.tsv         : all detected peaks with validation
#                                        columns, validation_status, and
#                                        grow_max_z.
#   <chr>_d17L1_envelopes.tsv          : L1 segments (boundary-derived
#                                        partition), one row per segment,
#                                        status="ENVELOPE".
#   <chr>_d17L1_boundary_score_curve.tsv : per-window score curve.
#
# What was removed (vs prior versions):
#   - Legacy envelope-discovery scan (scan_pass_l1 with W=800/400),
#     within-scale collapse, size-biased NMS, trim_to_core, post-trim NMS.
#     The boundary-driven L1 supersedes all of this.
#   - --l1_source / --pass1_W / --sim_thresh_* / --l1_density_min /
#     --nms_overlap / --scan_step / --min_n_windows / --trim_core /
#     --trim_drop_z / --post_trim_nms / --post_trim_nms_overlap
#     (all related to the legacy machinery).
#   - inside_L1 / L1_id columns on the boundaries TSV (referred to legacy
#     envelopes which no longer exist).
#
...
if (!dry_run && exists("bdt")) {
  ...
  if (length(cuts) == 0L) {
    ...
    seg_dt <- data.table(
      chr          = chr_label,
      candidate_id = sprintf("%s_d17L1_%04d", chr_label, seq_len(n_seg)),
      start_w      = seg_starts,
      end_w        = seg_ends,
