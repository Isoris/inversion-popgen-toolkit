...
if (8 %in% run_phases) {
  ...
  # BUGFIX 2026-04-17 (FIX 22, DESIGN): wire 2e GHSL v5 (C04 snake3
  ...
  #
  # Wiring strategy: C04 writes <chr>.ghsl_v5.annot.rds keyed by
  ...
  #
  # If --ghsl-dir is empty or no <chr>.ghsl_v5.annot.rds exists (Clair3
  # still running on this chromosome), the merge is a no-op. C01d handles
  # missing columns via safe_num()/%||% fallbacks.
  #
  # C01d's D10 will be updated in a paired patch to prefer ghsl_v5_score
  # over the sim_mat partition_stability when both are present.
  if (ARGS$ghsl_dir != "") {
    ...
    if (file.exists(ghsl_annot_file)) {
      cat("  Loading GHSL v5 annot:", ghsl_annot_file, "\n")
      ...
      if (!is.null(ghsl_annot) && nrow(ghsl_annot) > 0 &&
        ...
        for (ti in seq_len(nrow(tab))) {
          ...
          n_scored <- sum(!is.na(win_rows$ghsl_v5_score))
          ghsl_per_block$ghsl_n_scored_windows[ti] <- n_scored
          if (n_scored >= 3) {
            ghsl_per_block$ghsl_v5_score_max[ti] <-
              max(win_rows$ghsl_v5_score, na.rm = TRUE)
            if ("ghsl_rank_stability" %in% names(win_rows)) {
              ghsl_per_block$ghsl_rank_stability_max[ti] <-
                max(win_rows$ghsl_rank_stability, na.rm = TRUE)
            }
            if ("ghsl_div_contrast_z" %in% names(win_rows)) {
              ghsl_per_block$ghsl_div_contrast_z_max[ti] <-
                max(win_rows$ghsl_div_contrast_z, na.rm = TRUE)
            }
            if ("ghsl_div_bimodal" %in% names(win_rows)) {
              ghsl_per_block$ghsl_div_bimodal_frac[ti] <-
                mean(win_rows$ghsl_div_bimodal >= 2, na.rm = TRUE)
            }
            # PASS fraction — requires joining back to status-bearing track,
            # so we approximate via ghsl_v5_score > 0.65 as the PASS rule
            # C04 uses internally (see STEP_C04 L478-482)
            ghsl_per_block$ghsl_pass_frac[ti] <-
              mean(win_rows$ghsl_v5_score > 0.65, na.rm = TRUE)
          }
        }
        # Replace infinite max()/mean() on empty-after-na.rm with NA
        for (col in c("ghsl_v5_score_max", "ghsl_rank_stability_max",
        ...
        cat("  Merged GHSL v5 annotations for",
