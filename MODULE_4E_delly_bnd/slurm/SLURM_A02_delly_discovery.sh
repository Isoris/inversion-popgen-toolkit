...
dv_init_dirs
dv_log "=== STEP 2: Per-sample DELLY BND discovery ==="
...
run_discovery() {
  ...
  echo "[$(date '+%T')] ${sid}: starting BND discovery..."
...
while IFS= read -r sid; do
  [[ -f "${DIR_DISC}/${sid}.disc.BND.bcf" ]] || { dv_err "Missing: ${sid}"; ((FAIL++)) || true; }
done < "$SAMPLES_ALL"
...
{
  echo -e "sample\tn_BND"
  for bcf in "${DIR_DISC}"/*.disc.BND.bcf; do
    sid=$(basename "$bcf" .disc.BND.bcf)
    n=$(bcftools view -H "$bcf" 2>/dev/null | wc -l)
    echo -e "${sid}\t${n}"
  done
} | tee "${DIR_LOGS}/discovery_BND_counts.tsv"
