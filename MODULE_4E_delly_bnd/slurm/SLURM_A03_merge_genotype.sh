...
dv_init_dirs
dv_log "=== STEPS 3–8: Merge → Regenotype → Cohort → Germline filter (BND) ==="
...
run_genotype() {
  ...
  local out="${DIR_GENO}/${sid}.geno.BND.bcf"
...
if "${DELLY_BIN}" filter -f germline -o "${GERMLINE_81}" "${COHORT_81_TRIM}" \
  ...
  dv_log "  Germline PASS: $(bcftools view -H "${GERMLINE_81}" | wc -l) BNDs"
else
...
# 226 raw catalog
FINAL_226_RAW_VCF="${DIR_FINAL}/catalog_226.BND.raw.vcf.gz"
...
dv_log "  226 strict catalog: $(tail -n +2 "${MATRIX_226}" | wc -l) BNDs"
...
# 81 raw catalog
FINAL_81_RAW_VCF="${DIR_FINAL}/catalog_81.BND.${LABEL_81}.raw.vcf.gz"
bcftools view -O z -o "${FINAL_81_RAW_VCF}" "${INPUT_81}"
tabix -f -p vcf "${FINAL_81_RAW_VCF}"

# 81 strict catalog
FINAL_81_PASS="${DIR_FINAL}/catalog_81.BND.${LABEL_81}.PASS.vcf.gz"
if [[ "${STRICT_REQUIRE_PASS}" -eq 1 ]]; then
...
MATRIX_81="${DIR_FINAL}/catalog_81.BND.${LABEL_81}.PASS.GT_matrix.tsv"
{
