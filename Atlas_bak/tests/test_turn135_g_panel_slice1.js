...
// 6c. Inheritance tab body — confirmed counts
{
  console.log('\n--- 6c. Inheritance tab body (confirmed-count derivation) ---');
  // 0 confirmed
  let sb = makeGPanelSandbox({});
  let body = sb.ctx._gPanelRenderTabInheritance();
  ok('mentions "Slice 3 pending"', /Slice 3 pending/.test(body));
  ok('shows confirmed count = 0',
     /Confirmed candidates available for inheritance compute:\s*0/.test(body));
  ok('shows the < 2 empty-state hint',
     /will require ≥2 confirmed candidates/.test(body));
  ...
  ok('shows confirmed count = 3 (skips !confirmed)',
     /Confirmed candidates available for inheritance compute:\s*3/.test(body));
  ok('shows above-threshold hint when ≥ 2',
     /Threshold above the Slice 3 minimum/.test(body));
}
...

{
  ...
  ok('header mentions Slice 2 / Slice 3 placeholder framing',
     /Slice 2/.test(innerHTML) && /Slice 3/.test(innerHTML));
